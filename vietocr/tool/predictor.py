from vietocr.tool.translate import build_model, translate, translate_beam_search, process_input, predict, translate_v2
...
class Predictor():
    ...
    def predict(self, img, return_prob=False):
        ...
        if self.config['predictor']['beamsearch']:
            sents = translate_beam_search(img, self.model)
            prob = None
        else:
            sents, prob = translate_v2(img, self.model)

        sents = [self.vocab.decode(s) for s in sents.tolist()]
