...
from vietocr.tool.translate import translate, batch_translate_beam_search, translate_v2
...
from vietocr.loader.dataloader_v2 import OCRDataset, ClusterRandomSampler, Collator
...
class Trainer():
    def __init__(self, config, pretrained=True, augmentor=ImgAugTransform(), mode="train"):
        ...
        if mode == "train":
            ...
            self.train_gen = self.data_gen('train_{}'.format(self.dataset_name), 
                    self.data_root, self.train_annotation, self.config["dataset"]["train_label_studio_annotation"], self.masked_language_model, transform=transforms)
        
        
        self.valid_gen = self.data_gen('valid_{}'.format(self.dataset_name), 
                    self.data_root, self.valid_annotation, self.config["dataset"]["valid_label_studio_annotation"], masked_language_model=False)
    ...
    def train(self):
        ...
        for i in range(self.num_iters):
            ...
            if self.iter % self.valid_every == 0:
                ...
                print("Valid time: {}".format(time.time()-val_t))
...
#                loss = self.criterion(rearrange(outputs, 'b t v -> (b t) v'), rearrange(tgt_output, 'b o -> (b o)'))
    ...
    def predict(self, sample=None):
        ...
        img_files = []
        probs = []
        for batch in iter(self.valid_gen):
            batch = self.batch_to_device(batch)
            ...
            if self.beamsearch:
                ...
                translated_sentence, prob = translate_v2(batch['img'], self.model)
                probs.extend(prob)
    ...
    def data_gen(self, lmdb_path, data_root, annotation, label_studio_path, masked_language_model=True, transform=None):
        dataset = OCRDataset(lmdb_path=lmdb_path, 
                root_dir=data_root, annotation_path=annotation, 
                vocab=self.vocab, transform=transform, label_studio_path = label_studio_path,
                image_height=self.config['dataset']['image_height'], 
                image_min_width=self.config['dataset']['image_min_width'], 
                image_max_width=self.config['dataset']['image_max_width'],
                check_field = self.config['check_field'])
